...
class LoadingScreen(QWidget):
    ...
    def __init__(self):
        ...
        self.move((screen.width()-320)//2, (screen.height()-320)//2)
        self._build()
        self._phase = 0
        self._progress = 0.0
    ...
    def _build(self):
        ...
        card = QFrame()
        card.setStyleSheet(
            f"background: {P['bg2']}; border-radius: 24px;"
        ...
        logo.setFont(QFont("Segoe UI Emoji", 34))
        logo.setText("💬")
        logo.setStyleSheet(
            f"background: qlineargradient(x1:0,y1:0,x2:1,y2:1,"
            f"stop:0 {A()}, stop:1 #ff6b35);"
            f"border-radius: 20px; color: white;"
        )
        ...
        sub = QLabel("Roblox Overlay")
        sub.setFont(QFont("Segoe UI", 9))
        sub.setStyleSheet(f"color: {P['text2']};")
        sub.setAlignment(Qt.AlignCenter)
        inner.addWidget(sub)
        inner.addSpacing(32)

        # Spinner
        self.spinner = LoadingSpinner(40, A())
        inner.addWidget(self.spinner, 0, Qt.AlignCenter)
        inner.addSpacing(16)

        # Status
        self.status_lbl = QLabel("Initialising…")
        self.status_lbl.setFont(QFont("Segoe UI", 8))
        self.status_lbl.setStyleSheet(f"color: {P['text2']};")
        self.status_lbl.setAlignment(Qt.AlignCenter)
        inner.addWidget(self.status_lbl)
        inner.addSpacing(16)

        # Progress bar (thin)
        self.prog_track = QFrame()
        self.prog_track.setFixedHeight(3)
        self.prog_track.setStyleSheet(f"background: {P['bg4']}; border-radius: 2px;")
        inner.addWidget(self.prog_track)

        self.prog_fill = QFrame(self.prog_track)
        ...
        ver.setStyleSheet(f"color: {P['text2']}44; margin-top: 8px;")
        ver.setAlignment(Qt.AlignCenter)
        inner.addWidget(ver)

    def setStatus(self, text: str, progress: float):
        self.status_lbl.setText(text)
        self._progress = progress
        w = int(self.prog_track.width() * progress)
        self._animateProgress(w)

    def _animateProgress(self, target_w):
        anim = QVariantAnimation(self)
        anim.setStartValue(self.prog_fill.width())
        anim.setEndValue(target_w)
        anim.setDuration(300)
        anim.setEasingCurve(QEasingCurve.OutCubic)
        anim.valueChanged.connect(lambda v: self.prog_fill.setFixedWidth(int(v)))
        anim.start(QVariantAnimation.DeleteWhenStopped)

    def finish(self):
        self.setStatus("Ready!", 1.0)
        QTimer.singleShot(500, self._fadeOut)
    ...
    def _fadeOut(self):
        ...
        self._fade_anim.valueChanged.connect(lambda v: self.setWindowOpacity(v))
        self._fade_anim.finished.connect(self._onFadeFinished)
...
class FriendsPanel(QWidget):
    ...
    def _makeFriendRow(self, fr):
        ...
        dm_btn.clicked.connect(lambda: self.open_dm.emit(uname, ""))
        r.addWidget(dm_btn)
        return row
...
class RBXChat(QWidget):
    ...
    def _build_ui(self):
        ...
        self.dm_panel.open_dm.connect(self._openDM)
        self.friends_panel.open_dm.connect(self._openDM)
...
def main():
    ...
    def boot():
        steps = [
            (0.15, "Loading config…"),
            (0.35, "Connecting to Firebase…"),
            (0.55, "Loading profile…"),
            (0.75, "Starting overlay…"),
            (0.95, "Almost ready…"),
        ]
        for prog, label in steps:
            QTimer.singleShot(0, lambda p=prog, l=label: splash.setStatus(l, p))
            time.sleep(0.35)

        QTimer.singleShot(0, lambda: splash.finish())
    ...
    def on_splash_done():
        ...
        w.chat_panel.addMessage({
            "username": "RBXChat",
            "text": f"Welcome back, {p.get('username','stranger')}! 👋 Press F9 to toggle.",
            "timestamp": int(time.time()*1000)
        })
        w.show()
        w._slideIn()
